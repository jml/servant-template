...
module Logging (withLogging, log, info) where
...
withLogging :: (MonadIO m, Pretty msg) => LogM msg m a -> m a
...
printLogs :: (Pretty a, MonadIO m) => WithTimestamp (WithSeverity a) -> m ()
printLogs = print . renderWithTimestamp timeFormatter (renderWithSeverity pretty)
  where
